...
状态记录：~/.hermes/.liuyao_session.json
"""
...
# 状态文件路径
SESSION_FILE = os.path.expanduser("~/.hermes/.liuyao_session.json")
    ...
    try:
        ...
        if time.time() - last_update > TIMEOUT_SECONDS:
            ...
            try:
                ...
                pass
            return None
        # 未超时，更新last_update延长超时时间
        session["last_update"] = time.time()
        # 检查必要字段是否存在，缺失则初始化默认值
        # 注：如果缺失start_time，使用已记录的时间而非当前时间
        if "start_time" not in session:
            session["start_time"] = session.get("last_update", time.time())
        required_fields = {"step": 1, "yaos": [], "question": ""}
...
def save_session(session: Dict):
    ...
    with open(tmp_file, "w", encoding="utf-8") as f:
        ...
        try:
            ...
            fcntl.flock(f.fileno(), fcntl.LOCK_UN)
    os.replace(tmp_file, SESSION_FILE)  # 原子替换
...
def is_divination_request(user_input: str) -> tuple:
    ...
    has_question_word = any(w in user_input for w in ["吗", "么", "呢", "如何", "怎么样", "能不能", "可不可以", "行不行", "好不好", "能否", "问", "问问", "是不是", "是否有"])
...
def start_divination(question: str = ""):
    """
    开始新的起卦流程
    只创建session，不自动摇卦——所有6次摇卦都由用户说「心想事成」触发
    """
    clear_session()
...
def process_one_step(user_input: str, session: Dict) -> tuple:
    ...
    # 用 yaos 长度推断当前第几次（避免step字段漂移）
    current_count = len(yaos)
    ...
    # 记录爻
    yao_info = {
        "yao_name": YAO_NAMES[current_count],  # 0-based → 对应正确的爻名
        "type": yao_type,
        "symbol": symbol,
        "dong": is_dong,
    ...
    # 构建响应（紧凑格式）
    response = f"第{next_step}次（{YAO_NAMES[current_count]}）：{format_coins(coins)} → {yao_type} {symbol}"
    ...
    if new_count < 6:
        # 继续下一步
        save_session(session)
        response += f"\n\n请发送「{TRIGGER_WORD}」继续（第{new_count + 1}次/共6次）"
...
def process_step(user_input: str) -> str:
    ...
    if session is None:
        ...
        if user_input == TRIGGER_WORD:
            return start_divination("")
        return "当前没有进行中的起卦。发送「起卦」或您的问题开始新的摇卦。"
    
    # 有活跃会话，必须说「心想事成」才能推进
    if user_input != TRIGGER_WORD:
        return f"❌ 请发送「{TRIGGER_WORD}」继续摇卦"
...
def complete_divination(session: Dict) -> str:
    ...
    # 构建给liuyao_interpret.py的输入格式
    interpret_input = build_interpret_input(yaos, question)
    
    # 调用美颜版解读脚本
    interpret_result = interpret_direct(interpret_input, pretty=True)
...
def get_status() -> str:
    ...
    step = len(session.get("yaos", [])) + 1  # 基于实际爻数，不依赖step字段
    yaos = session["yaos"]
    question = session.get("question", "")
...
def interpret_direct(user_input: str, pretty: bool = False) -> str:
    """直接解读模式（保留原有功能）"""
...
def handle_user_input(user_input: str) -> str:
    ...
    输入判定规则（严格版）：
    1. 「心想事成」→ 继续摇卦，每次只处理1步
    2. 其他任意文字或emoji → ❌ 提示请发送「心想事成」继续摇卦

    场景：
    1. 有活跃会话 + 心想事成 → 处理1次摇卦
    2. 有活跃会话 + 其他任何输入 → ❌ 请发送「心想事成」继续摇卦
    3. 无会话 + 起卦请求 → 开始新起卦
    4. 无会话 + 其他输入 → 返回提示
    """
    ...
    # 特殊处理：无会话 + 心想事成 → 创建session并直接摇第1次
    if user_input == TRIGGER_WORD:
        start_divination("")
        return process_step(user_input)

...
class LiuYaoInterpreter:
    ...
    def get_current_day_gan_zhi(self) -> Tuple[str, str]:
        ...
        zhi_idx = (4 + days_diff) % 12  # 基准日甲辰日，辰在zhi_list[4]
    ...
    def parse_input(self, user_input: str) -> bool:
        ...
        # 检查第一行是否为爻行（无question时，第一行可能是爻）
        first_is_yao = bool(re.match(r"^[上五四三二初]爻[：:]\s*[\u4e00-\u9fa5]+", lines[0].strip()))
        ...
        yao_lines = []
        for line in lines[yao_start_idx:]:
            line = line.strip()
        ...
        if len(yao_lines) != 6:
            ...
            yao_lines = []
            for line in lines[yao_start_idx + 1:]:
        ...
        for i, line in enumerate(yao_lines):
            ...
            if match:
                ...
                # 判断是否为动爻：只有老阳/老阴才是动爻
                is_dong = yao_type in ["老阳", "老阴"]
                
                pos_map = {"初": 0, "二": 1, "三": 2, "四": 3, "五": 4, "上": 5}
                pos_idx = pos_map[yao_pos]  # regex已保证yao_pos合法
    ...
    def analyze_wangshuai(self, hexagram_info: Dict) -> Dict:
        ...
        # 优先使用设置的日支，否则从干支历获取
        if not self.day_zhi or not self.day_gan:
            self.day_gan, self.day_zhi = self.get_current_day_gan_zhi()
    ...
    def check_patterns(self, hexagram_info: Dict) -> List[str]:
        ...
        for p_name, p_list in liushou.items():
            # 子串匹配：数据中卦名为全名（如"地水师"），规则库是短名（如"师"）
            if any(short_name in hexagram_name for short_name in p_list):
    ...
    def interpret(self, user_input: str, pretty: bool = False) -> str:
        """主解读入口"""
        ...
        if pretty:
            return self.generate_pretty_interpretation()
        return self.generate_interpretation()

    def generate_pretty_interpretation(self) -> str:
        """美颜版解读 - emoji直观呈现"""
        output = []
        ...
        # 爻类型emoji
        YAO_EMOJI = {"老阳": "⚊", "老阴": "⚋", "少阳": "⚊", "少阴": "⚋"}

        output.append("## 🎯 六爻起卦\n")
        output.append(f"**起卦时间**：{date_str}\n\n")

        # 卦象总览
        output.append("### 🔮 卦象\n\n")
        main_name = h_info.get("name", "?")
        main_gong = h_info.get("gong", "?")
        main_pos = h_info.get("position", "")
        
        # 动爻标记
        dong_count = len(self.dong_yao_indices)
        dong_tag = f"{dong_count}动爻" if dong_count > 0 else "静卦"
        output.append(f"**{main_name}** · {main_gong}{main_pos} · {dong_tag}\n\n")
        
        # emoji 一排排展示卦象（从初爻到上爻）
        yao_symbols = []
        for yao in self.yao_results:
            sym = YAO_EMOJI.get(yao["type"], "?")
            yao_symbols.append(sym)
        output.append(" ".join(yao_symbols) + "\n\n")

        bian_name = bian_info.get("name", "?") if bian_info and bian_info.get("name") != "变卦" else "—"
        if bian_name != "—":
            output.append(f"**变卦**　{bian_name}\n\n")
        output.append("---\n\n")

        # 六次摇卦
        output.append("### 🎲 六次摇卦\n\n")
        # 爻位顺序
        yao_order = ["初", "二", "三", "四", "五", "上"]

        # 读取硬币记录（从session文件）
        coin_list = []
        session_path = os.path.expanduser("~/.hermes/.liuyao_session.json")
        if os.path.exists(session_path):
            try:
                with open(session_path, "r", encoding="utf-8") as f:
                    session = json.load(f)
                    yaos = session.get("yaos", [])
                    coin_list = [yao.get("coins", "⚪⚪⚪") for yao in yaos]
            except:
                pass

        for i, yao in enumerate(self.yao_results):
            idx = i + 1
            yao_name = self.YAO_NAMES[i]

            # 硬币emoji
            if i < len(coin_list):
                coin_str = coin_list[i]
            else:
                coin_str = "⚪⚪⚪"

            # 爻符号和类型
            yao_symbol = YAO_EMOJI.get(yao["type"], "?")
            yao_type = yao["type"]
            is_dong = yao["dong"]
            dong_mark = " ← 动爻 ✨" if is_dong else ""

            output.append(f"第{idx}次  {coin_str}  →  {yao_symbol} {yao_type}{dong_mark}\n")
        output.append("\n")

        # 卦辞
        gua_ci = h_info.get("gua_ci", "暂无")
        xiang_ci = h_info.get("xiang_ci", "暂无")
        output.append("### 📜 卦辞\n\n")
        output.append(f"**{main_name}** · {main_gong}{main_pos}\n\n")
        output.append(f"> \"{gua_ci}\"\n\n")
        output.append(f"*象曰：{xiang_ci}*\n\n")

        # 用神选取
        yongshen = self.get_yongshen()
        if self.question:
            output.append(f"### 🔍 用神\n\n")
            output.append(f"所问：{self.question}\n")
            output.append(f"用神：**{yongshen.get('primary', '?')}**\n")
            if yongshen.get('secondary'):
                output.append(f"辅神：{'、'.join(yongshen['secondary'])}\n")
            output.append(f"\n")
        ...
        if self.dong_yao_indices:
            ...
            for pos_idx in self.dong_yao_indices:
                ...
                liuqin = naga.get('liuqin', '?')
                zhi = naga.get('zhi', '?')
                wx = naga.get('zhi_wuxing', '?')
                output.append(f"- {yao['yao_name']}：{old_yin_yang}→{new_yin_yang}（{liuqin}·{zhi}{wx}）\n")
            output.append(f"\n")

        # 旺衰详情
        wangshuai = self.analyze_wangshuai(h_info)
        month_zhi = wangshuai.get('month_zhi', '?')
        yueling = wangshuai.get('yueling_wangxiang', '?')
        day_gan_zhi = wangshuai.get('day_gan_zhi', '?')
        
        output.append("### 🌙 旺衰\n\n")
        output.append(f"月令：{month_zhi}月（{yueling}）\n")
        output.append(f"日建：{day_gan_zhi}日\n\n")

        yongshen_analysis = wangshuai.get("yongshen_analysis", [])
        if yongshen_analysis:
            output.append(f"**用神旺衰**：\n")
            for a in yongshen_analysis:
                dong_tag = " ⚡动" if a.get('is_dong') else ""
                output.append(f"- {a['yao_name']}（{a['liuqin']}·{a['zhi']}{a['wuxing']}）：**{a['overall']}**{dong_tag}\n")
            output.append(f"\n")
        ...
        if dong_count == 0:
            jiqiong_emoji = "⚖️"
            jiqiong_text = "平"
            dynamic_desc = "静卦，事态平稳"
        elif dong_count == 1:
            jiqiong_emoji = "⚖️"
            jiqiong_text = "平"
            dynamic_desc = f"{dong_count}个动爻，留意变化"
        elif dong_count == 2:
            jiqiong_emoji = "⚖️"
            jiqiong_text = "平偏吉"
            dynamic_desc = f"{dong_count}个动爻，事态多变"
        else:
            jiqiong_emoji = "🚨"
            jiqiong_text = "谨慎"
            dynamic_desc = f"{dong_count}个动爻，变化较多"

        # 用神旺衰（复用前面🌙旺衰已计算的结果）
        yongshen_analysis = wangshuai.get("yongshen_analysis", [])
        wangshuai_note = ""
        if yongshen_analysis:
            if any("旺" in a.get("overall", "") for a in yongshen_analysis):
                wangshuai_note = "用神旺相有力"
            elif any("休囚" in a.get("overall", "") for a in yongshen_analysis):
                wangshuai_note = "用神休囚无力"
            else:
                wangshuai_note = "用神旺衰平和"

        output.append("### ⚖️ 吉凶\n\n")
        output.append(f"{jiqiong_emoji} **吉凶**：{jiqiong_text} ｜ {dynamic_desc}\n\n")
        if wangshuai_note:
            output.append(f"{wangshuai_note}\n\n")
        ...
        if self.day_gan:
            ...
            for i in range(5, -1, -1):  # 上爻到初爻
                liushou = self.get_liushou_for_yao(i)
                output.append(f"- {self.YAO_NAMES[i]}：{liushou}\n")
            output.append("\n")
        ...
        output.append("### 💡 建议\n\n")
        output.append(f"**⏰ 时机**　{timing}\n\n")

        # 优势
        output.append("**✅ 优势**\n")
        output.append("- 顺应规律，顺其自然为上\n\n")

        # 挑战
        output.append("**⚠️ 注意**\n")
        if self.dong_yao_indices:
            output.append(f"- 存在{dong_count}个动爻，有突发变化可能\n")
            output.append("- 方向未明时忌冒进\n")
        else:
            output.append("- 事态平稳，但可能缺乏突破动力\n")
        output.append("- 保持定力，理性分析再做决定\n\n")

        # 行动
        output.append("**🎯 行动**\n")
        output.append("- 先做好眼前事，不急于求成\n")
        if self.dong_yao_indices:
            output.append("- 准备备选方案应对变化\n")
        output.append("- 用结果证明能力\n\n")
...
def main():
    ...
    if len(sys.argv) > 1 and not pretty:
    ...
    result = interpreter.interpret(user_input, pretty=pretty)
    print(result)

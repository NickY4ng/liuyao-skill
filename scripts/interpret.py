...
from naga import analyze, print_analysis, get_biangua_yaos, BAGUANAGA
...
def interpret(question, yao_list, month_zhi=None):
    ...
    output.append(f"  卦名：{result['hexagram_name']}  卦宫：{result['palace']}")
    ...
    if result["moving_indices"]:
        output.append(f"  动爻：{', '.join([yao_cn[i] for i in result['moving_indices']])}")
        output.append(f"  变卦：{result['biangua_name']}（{result['biangua_id']}）")
